...
describe("EthState", function () {
  ...
  describe("Lender getting financing info", function () {
    it("should get all financing info", async function () {
      const {
        property,
        financing,
        owner,
      ...
      // user1 adds a loan
      await expect(
        financing.connect(user1).addLoan(user1.address, 500, 12)
      ).to.emit(financing, "LoanAdded");
      ...
      // user2 bids on the property
      await expect(property.connect(user2).bid(1, 1_000_000)).to.emit(
        property,
        "Offer"
      );
      await expect(property.acceptOffer(1, user2.address)).to.emit(
        property,
        "Accept"
      );

      // user2 requests financing
      await expect(
        property.connect(user2).requestFinancing(1, 1, 1_000_000, 12)
      ).to.emit(financing, "FinanceRequest");

      // user1 approves financing
      await expect(
        financing.connect(user1).approveFinancing(1, { value: 1_000_000 })
      ).to.emit(financing, "FinanceApproval");

      // user1 get all financing info
      const financingInfo = await financing
        .connect(user1)
        .lenderGetFinancings();
      expect(financingInfo.length).to.equal(1);
      expect(financingInfo[0].loaner).to.equal(user2.address);
    });
  });
});
